...
- 실제 인증, OTP 화면 이동, 잔액 차감과 송금은 수행하지 않는다.
...
D11은 세 기본 화면, 메인 업무 이동, 상품·출금 계좌의 로컬 단일 선택,
예금 상품 상세, 가입 금액 로컬 검증, 예금 약관 개별 선택과 수취인 로컬
선택·확인, 이체 금액 로컬 검증과 계좌 비밀번호 보안 입력 Mock까지
제공한다. 실제 예금 가입, 송금, 잔액 차감, 인증, OTP, API, WebSocket과
외부 금융사이트 연결은 구현하지 않는다.
